...
🧮 El Modelo MatemáticoEl programa calcula la probabilidad final de un resultado $R \in \{1, X, 2\}$ mediante una media ponderada normalizada.
Primero, calcula el peso de cada fuente $W_i$ basándose en su tasa de aciertos histórica $T_i$:$$W_i = \frac{T_i}{\sum_{j=1}^{N} T_j}$$
Posteriormente, la probabilidad consolidada del sistema se calcula sumando el producto de las probabilidades individuales de cada fuente $P_i(R)$ por su peso asignado:$$P_{final}(R) = \sum_{i=1}^{N} W_i \cdot P_i(R)$$
📂 Estructura del Proyecto
1X2-predictor/
│
├── data/                       # 📁 Datos JSON (Ignorados en control de versiones)
│   ├── mis_fuentes.json        # Base de datos histórica de aciertos/fallos
│   ├── entrada_viernes.json    # Cuotas y probabilidades de la jornada actual
│   └── resultados_lunes.json   # Resultados reales para retroalimentar el modelo
│
...
1. Preparación y Cálculo (Viernes)
Rellena el archivo data/entrada_viernes.json con los partidos de la jornada y las cuotas/probabilidades de tus fuentes (ej. Pinnacle, Opta, Forebet).
Ejecuta el calculador:
Bash
python viernes_calcular.py
El programa imprimirá en consola los porcentajes consolidados y precisos para cada posible resultado (1, X, 2).2. Retroalimentación del Modelo (Lunes)
Tras finalizar la jornada, abre data/resultados_lunes.json y sustituye las incógnitas por los resultados reales ("1", "X" o "2").
Ejecuta el actualizador:
Bash
python lunes_actualizar.py
El sistema evaluará las predicciones hechas el viernes, sumará los aciertos/fallos, actualizará el archivo mis_fuentes.json y mostrará el nuevo ranking de fiabilidad de tus fuentes.
